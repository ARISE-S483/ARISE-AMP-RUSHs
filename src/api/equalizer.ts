...
const equalizerSettings = {
    getBandCount: () => useEqualizerStore.getState().bandCount,
    setBandCount: (c: number) => useEqualizerStore.getState().setBandCount(c),
    getFreqRange: () => ({ min: useEqualizerStore.getState().freqMin, max: useEqualizerStore.getState().freqMax }),
    setFreqRange: (min: number, max: number) => useEqualizerStore.getState().setFreqRange(min, max),
    getGains: () => useEqualizerStore.getState().gains,
    setGains: (gains: number[]) => useEqualizerStore.getState().setGains(gains),
    interpolateGains: (oldGains: number[], newCount: number) => useEqualizerStore.getState().interpolateGains(oldGains, newCount),
    isEnabled: () => useEqualizerStore.getState().enabled,
    setEnabled: (enabled: boolean) => useEqualizerStore.getState().setEnabled(enabled),
    getPreamp: () => useEqualizerStore.getState().preamp,
    setPreamp: (preamp: number) => useEqualizerStore.getState().setPreamp(preamp),
    setPreset: (preset: string) => useEqualizerStore.getState().setPreset(preset),
    getRange: () => ({ min: -30, max: 30 }),
    setBandTypes: (types: string[]) => useEqualizerStore.getState().setBandTypes(types),
    setBandQs: (qs: number[]) => useEqualizerStore.getState().setBandQs(qs),
    setCustomFrequencies: (freqs: number[]) => useEqualizerStore.getState().setCustomFrequencies(freqs),
    MIN_BANDS: 3,
    MAX_BANDS: 32,
    DEFAULT_PREAMP: 0
};
