...
const DEFAULT_INSTANCES: InstanceEntry[] = [
  ...
  // Piped (YouTube Music)
  { url: 'https://api.piped.projectsegfau.lt', type: 'streaming', enabled: true },
  { url: 'https://pipedapi.smnz.de', type: 'streaming', enabled: true },
  { url: 'https://pipedapi.kavin.rocks', type: 'streaming', enabled: true },
  { url: 'https://pipedapi.adminkong.com', type: 'streaming', enabled: true },
  // Deezer (RapidAPI) — grouped under YouTube/Piped
  { url: 'https://deezerdevs-deezer.p.rapidapi.com', type: 'streaming', enabled: true },
  // FreeYourMusic / MusicAPI13 (RapidAPI) — grouped under YouTube/Piped
  { url: 'https://musicapi13.p.rapidapi.com', type: 'streaming', enabled: true },
  // Shazam (RapidAPI) — grouped under YouTube/Piped
  { url: 'https://shazam.p.rapidapi.com', type: 'streaming', enabled: true },
  // Spotify23 (RapidAPI) — grouped under YouTube/Piped
  { url: 'https://spotify23.p.rapidapi.com', type: 'streaming', enabled: true },
  // BhaskarPanja93/MusicAPI — Live public music API (search, fetch, audio stream)
  { url: 'https://bhindi1.ddns.net/music/api', type: 'streaming', enabled: true },

  // ──────────────────────────────────────────────────────────────
  // YouTube Music Internal API endpoints (extracted from sigma67/ytmusicapi)
  // Base: https://music.youtube.com/youtubei/v1/
  // API Key: AIzaSyC9XL3ZjWddXya6X74dJoCTL-WEYFDNX30
  // ──────────────────────────────────────────────────────────────
  // browse — Home, Artist, Album, Playlist, Lyrics, Charts, Explore, Moods
  { url: 'https://music.youtube.com/youtubei/v1/browse', type: 'streaming', enabled: true },
  // player — Song/video metadata + streaming URLs
  { url: 'https://music.youtube.com/youtubei/v1/player', type: 'streaming', enabled: true },
  // search — Search YouTube Music (songs, albums, artists, playlists, videos)
  { url: 'https://music.youtube.com/youtubei/v1/search', type: 'streaming', enabled: true },
  // next — Watch playlist / Up Next queue / radio
  { url: 'https://music.youtube.com/youtubei/v1/next', type: 'streaming', enabled: true },
  // music/get_search_suggestions — Search autocomplete suggestions
  { url: 'https://music.youtube.com/youtubei/v1/music/get_search_suggestions', type: 'streaming', enabled: true },
  // feedback — Like/dislike/remove search history
  { url: 'https://music.youtube.com/youtubei/v1/feedback', type: 'streaming', enabled: true },
  // playlist/create — Create new playlist
  { url: 'https://music.youtube.com/youtubei/v1/playlist/create', type: 'streaming', enabled: true },
  // playlist/delete — Delete a playlist
  { url: 'https://music.youtube.com/youtubei/v1/playlist/delete', type: 'streaming', enabled: true },
  // browse/edit_playlist — Edit/add/remove playlist items
  { url: 'https://music.youtube.com/youtubei/v1/browse/edit_playlist', type: 'streaming', enabled: true },

  // ──────────────────────────────────────────────────────────────
  // YouTube Music OAuth & Auth endpoints (from sigma67/ytmusicapi constants)
  // ──────────────────────────────────────────────────────────────
  // OAuth device code — used for OAuth authentication flow
  { url: 'https://www.youtube.com/o/oauth2/device/code', type: 'streaming', enabled: true },
  // OAuth token — exchange auth code for access token
  { url: 'https://oauth2.googleapis.com/token', type: 'streaming', enabled: true },

  // YouTube Data API v3 — official Google API for YouTube content
  { url: 'https://www.googleapis.com/youtube/v3', type: 'streaming', enabled: true },
  // Invidious API — YouTube proxy (privacy-focused alternative)
  { url: 'https://inv.nadeko.net', type: 'streaming', enabled: true },
  { url: 'https://invidious.snopyta.org', type: 'streaming', enabled: true },
  // Beatbump API — YouTube Music web client alternative
  { url: 'https://beatbump.io/api', type: 'streaming', enabled: true },
  // Hyperpipe API — YouTube Music proxy  
  { url: 'https://hyperpipeapi.onrender.com', type: 'streaming', enabled: true },
  // YouTube Music (Server-side API via ytmusic-api npm) — local proxy
  { url: '/api/ytmusic', type: 'streaming', enabled: true },
  // JioSaavn
  { url: 'https://saavn.dev', type: 'jiosaavn', enabled: true },
  { url: 'https://saavn.me', type: 'jiosaavn', enabled: true },
  { url: 'https://jiosaavn-api-privatecvc2.vercel.app', type: 'jiosaavn', enabled: true },
  { url: 'https://jiosaavn-api-tau-three.vercel.app', type: 'jiosaavn', enabled: true },
  { url: 'https://jiosaavn-api-2a2b.onrender.com', type: 'jiosaavn', enabled: true },
];
      ...
      <div className="mb-6 p-4 rounded-xl border border-primary/20 bg-primary/5 space-y-3">
        <div>
          <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
            <Key size={14} className="text-primary" /> API Keys
          </h3>
          <p className="text-xs text-muted-foreground mt-1">
            Pre-built RapidAPI key for Deezer, FreeYourMusic, YouTube Music, Shazam & Spotify23 streaming sources.
          </p>
        </div>

        {/* RapidAPI Key */}
        <div>
          <label className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mb-1 block">RapidAPI Key</label>
          <div className="flex gap-2 items-center">
            <input
              ...
              className="flex-1 bg-background border border-border/50 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 text-foreground font-mono"
            />
            <button
              onClick={() => setShowKey(!showKey)}
              className="p-2 rounded-lg bg-secondary/60 hover:bg-accent transition-colors text-muted-foreground hover:text-foreground"
        ...
        {rapidapiKey && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-primary font-medium">Key configured — Deezer, FreeYourMusic, Shazam & Spotify23 are active</span>
          </div>
        )}
        {!rapidapiKey && (
          <div className="flex items-center gap-2">
            <AlertCircle size={12} className="text-yellow-500" />
            <span className="text-[10px] text-yellow-500 font-medium">No key set — RapidAPI-based sources will be disabled</span>
          </div>
        )}

        <p className="text-[10px] text-muted-foreground/70">
          The pre-built key enables seamless streaming from Deezer, FreeYourMusic, YouTube Music, Shazam & Spotify23 APIs.
          Get your own key at{' '}
          <a href="https://rapidapi.com" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
            rapidapi.com
          </a>
        </p>
      </div>

      {/* RapidAPI requirement notice for relevant tabs */}
      {needsRapidApi && !rapidapiKey && (
        <div className="mb-4 p-3 rounded-lg border border-yellow-500/20 bg-yellow-500/5">
          <p className="text-xs text-yellow-500 flex items-center gap-2">
            <Key size={12} />
            This source requires a RapidAPI key. Configure it above to enable.
          </p>
        </div>
      )}
