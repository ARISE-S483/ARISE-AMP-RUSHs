...
import {
  ...
  CheckCircle2, XCircle, GripVertical, Trash2, Plus, Youtube, Music2
} from 'lucide-react';
...
const DEFAULT_INSTANCES: InstanceEntry[] = [
  ...
  { url: 'https://pipedapi.leptons.xyz', type: 'streaming', enabled: true },
  { url: 'https://piped-api.lunar.icu', type: 'streaming', enabled: true },
  { url: 'https://pipedapi.colinslegacy.com', type: 'streaming', enabled: true },
  { url: 'https://pa.il.ax', type: 'streaming', enabled: true },
  ...
  // YouTube Music (Server-side API) — grouped under YouTube/Piped
  ...
  // BhaskarPanja93/MusicAPI — Live public music API
  { url: 'https://bhindi1.ddns.net/music/api', type: 'streaming', enabled: true },

  // YouTube Music Internal API endpoints (sigma67/ytmusicapi)
  ...
  // YouTube Music OAuth endpoints (sigma67/ytmusicapi constants)
  ...
  // Beatbump API — YouTube Music alternative
        ...
        try {
          ...
          if (activeType === 'api') {
            ...
            if (isRapidApiUrl) {
              ...
              // (Don't check RapidApi keys strictly on page, but normally requires X-RapidAPI headers to truly pass)
            } else if (
    ...
    setInstances(prev => {
      const next = prev.map(inst => {
        ...
        const result = results.find((r, idx) =>
          r.status === 'fulfilled' && filtered[idx]?.url === inst.url
        );
        if (result && result.status === 'fulfilled') {
          return {
            ...
            version: result.value.version || inst.version,
  ...
  return (
    <motion.div className="p-4 md:p-6 max-w-4xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="font-display text-xl md:text-2xl font-bold">Instances</h1>
      </div>
      <p className="text-sm text-muted-foreground mb-6">
        Manage TIDAL API instances priority, enable/disable, and check health status. Drag to reorder priority.
      </p>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            disabled={checking}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-secondary/60 hover:bg-accent transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-secondary/60 hover:bg-accent transition-colors"
          >
            <Plus size={12} />
            Add
          </button>
      ...
      <div className="glass-card rounded-xl p-3 md:p-4">
        {filtered.length === 0 ? (
          <div className="text-center py-12">
            <AlertCircle size={28} className="text-muted-foreground/30 mx-auto mb-3" />
            <p className="text-muted-foreground text-sm">No instances configured</p>
            <button
              onClick={() => setIsAdding(true)}
      ...
      <p className="text-[11px] text-muted-foreground/60 mt-4 text-center">
        Instances at the top have higher priority. Drag to reorder. The app will automatically failover to the next instance if one is unavailable.
      </p>
    </motion.div>
  );
}
